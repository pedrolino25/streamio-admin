...
export function formatConfiguration(config?: ProcessingConfiguration): string {
  if (!config) return "—";
  const parts: string[] = [];
  ...
  if (config.imageQuality) {
    parts.push(`Img Quality: ${config.imageQuality}`);
  }
  if (config.imageMaxWidth || config.imageMaxHeight) {
    parts.push(`Max Size: ${config.imageMaxWidth || "?"}x${config.imageMaxHeight || "?"}`);
  }
