...
import { validateAwsConfig } from "@/lib/utils/validation";
...
export function handleCredentialError(
  ...
  const errorContext: Record<string, unknown> = {
    ...
    region: config.region,
    userPoolId: config.userPoolId,
    providerName: `cognito-idp.${config.region}.amazonaws.com/${config.userPoolId}`,
  };
  ...
  if (isProviderNameMismatch) {
    ...
    throw new ApplicationError(
      ...
      {
        details: `The Identity Pool '${config.identityPoolId}' exists, but the provider_name doesn't match.`,
        originalError: error,
      }
    );
  }
